...
if (array_key_exists("g_mail", $tvc_data) && sanitize_email($tvc_data["g_mail"]) && isset($_GET['subscription_id']) && sanitize_text_field($_GET['subscription_id'])) {
    update_option('convsst_customer_gmail', sanitize_email($tvc_data["g_mail"]));
            ...
            <div class="tvc_google_signinbtn google-btn">
                <div class="google-icon-wrapper">
                    <img class="google-icon"
                        src="<?php echo esc_url(CONVSST_PLUGIN_URL . '/admin/images/g-logo.png'); ?>" />
                </div>
                <p class="btn-text"><b>
                        <?php esc_html_e("Sign in with google", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                    </b></p>
            </div>
        </div>
    <?php } ?>
        ...
        <div class="onbrdnpp-cntner acccretppcntnr">
            <div class="onbrdnpp-hdr">
                <div class="ppclsbtn clsbtntrgr"><img
                        src="<?php echo esc_url(CONVSST_PLUGIN_URL . '/admin/images/close-icon.png'); ?>" alt="" />
                </div>
            </div>
                    ...
                    <?php } else { ?>
                        <?php if ($is_refresh_token_expire == true) { ?>
                            <p class="alert alert-primary">
                                <?php esc_html_e("It seems the token to access your Google accounts is expired. Sign in again to continue.", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                            </p>
                            <div class="google_connect_url google-btn">
                                <div class="google-icon-wrapper">
                                    <img class="google-icon"
                        ...
                        <?php } else { ?>
                            <div class="google_connect_url google-btn">
                                ...
                                <p class="btn-text mr-35"><b>
                                        <?php esc_html_e("Reauthorize", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                                    </b></p>
                            </div>
                        <?php } ?>
                    <?php } ?>
                    <p>
                        <?php esc_html_e("Make sure you sign in with the google email account that has all privileges to access google analytics, google ads and google merchant center account that you want to configure for your store.", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                    </p>
                </div>
                <div class="google_signin_sec_right">
                    <h5>
                        <?php esc_html_e("Why do I need to sign in with google?", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                    </h5>
                    <p>
                        <?php esc_html_e("When you sign in with Google, we ask for limited programmatic access for your accounts in order to automate below features for you:", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                    </p>
                    <p><strong>
                            <?php esc_html_e("1. Google Analytics:", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                        </strong>
                        <?php esc_html_e("To give you option to select GA accounts, to show actionable google analytics reports in plugin dashboard and to link your google ads account with google analytics account.", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                    </p>
                    <p><strong>
                            <?php esc_html_e("2. Google Ads:", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                        </strong>
                        <?php esc_html_e("To automate dynamic remarketing, conversion and enhanced conversion tracking and to create performance campaigns if required.", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                    </p>
                    <p><strong>
                            <?php esc_html_e("3. Google Merchant Center:", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                        </strong>
                        <?php esc_html_e("To automate product feed using content api and to set up your GMC account.", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                    </p>

                </div>
                <!--badge consent & toggle -->
                <div style="margin-top: 10px; display:none" class="d-none">
                    <label id="badge_label_check" for="convsst_show_badge_onboardingCheck"
                        class="switch <?php echo empty($ee_options['convsst_show_badge']) || esc_attr($ee_options['convsst_show_badge']) == "no" ? "convsst_default_cls_disabled" : "convsst_default_cls_enabled"; ?>">
