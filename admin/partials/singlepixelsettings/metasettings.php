...
<div class="convcard p-4 mt-0 rounded-3 shadow-sm mt-3">
    ...
    <div class="alert d-flex align-items-cente p-0" role="alert">
        ...
        <div class="p-2 w-100 rounded-end border border-start-0 shadow-sm conv-notification-alert lh-lg">
            <?php esc_html_e("Meta Business Account", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
        </div>
    </div>
        ...
        <div id="analytics_box_UA" class="py-1 row">
            <div class="col-5">
                <label class="text-dark">
                    <?php esc_html_e("Facebook Business ID", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                </label>
            ...
            <div class="col-5">
                <label class="text-dark">
                    <?php esc_html_e("Facebook Catalog ID", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
                </label>
                <div class="pt-2 conv-metasettings">
                    <div class="col-12">
...
<div class="convcard p-4 mt-0 rounded-top shadow-sm mt-3">
    <div class="d-flex mt-0">
        ...
        <h6 class="m-0 fw-normal ms-2">
            <?php esc_html_e("Benefits of using your own GTM", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
        </h6>
    </div>
    <div class="d-flex mt-3">
        <label class="fs-5">
            <?php esc_html_e("Simplify your tag management with features like workspaces, tag templates, and so much more.", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
        </label>
    </div>
    <div class="d-flex mt-3">
    ...
    <div class="row">
        <div class="text-end col-md-6 p-0">
            <h6 class="fw-normal text-primary">
                <?php esc_html_e("Upgrade to Pro", "server-side-tagging-via-google-tag-manager-for-wordpress"); ?>
            </h6>
        </div>
...
function list_google_merchant_account(tvc_data, selelement) {
    var selectedValue = '". esc_js($google_merchant_center_id). "';
    var convsst_onboarding_nonce = '". esc_js(wp_create_nonce('convsst_onboarding_nonce')). "';
    jQuery.ajax({
        type: 'POST',
        dataType: 'json',
        url: tvc_ajax_url,
        data: { action: 'list_google_merchant_account', tvc_data: tvc_data, convsst_onboarding_nonce: convsst_onboarding_nonce },
...
jQuery(function () {
    ...
    let google_merchant_center_id = '". esc_attr($google_merchant_center_id). "';
        ...
        if (has_error == 1) {
            ...
            jQuery.ajax({
                ...
                data: {
                    ...
                    convsst_options_data: JSON.stringify(selected_vals),
                },
                beforeSend: function () {
                ...
                success: function (response) {
                    ...
                    if (response == '0' || response == '1') {
                        ...
                        jQuery('#convsst_save_success_txt').html('Congratulations, you have successfully saved your <br> Google Tag Manager account with <br> ' + user_modal_txt);
