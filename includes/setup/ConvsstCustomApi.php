...
class ConvsstCustomApi
{
  ...
  public function get_tvc_access_token()
  {
    if (!empty($this->access_token)) {
      ...
      if ((isset($google_detail['setting']->access_token))) {
        $this->access_token = base64_decode($google_detail['setting']->access_token);
      }
      return $this->access_token;
    }
  }
  ...
  {
    $ee_options_data = unserialize(get_option('convsst_options'));
  ...
  public function update_app_status($status = 1)
  {
    try {
      ...
      if ($subscription_id != "") {
        ...
        $options = unserialize(get_option('convsst_options'));
        $fb_pixel_enable = "0";
        ...
        $attributes = unserialize(get_option('convsst_prod_mapped_attrs'));
        $categories = unserialize(get_option('convsst_prod_mapped_cats'));
        ...
        $postData = array(
          ...
          "app_status_data" => array(
            "app_settings" => array(
              ...
              "app_verstion" => CONVSST_PLUGIN_VERSION,
              "domain" => esc_url(get_site_url()),
              "product_settings" => unserialize(get_option('convsst_options')),
    ...
    try {
      ...
      if (isset($subscription_id) && $status != "") {
        ...
        $postData = array(
          ...
          "app_status" => sanitize_text_field($status),
          "app_data" => array(
            "app_version" => CONVSST_PLUGIN_VERSION,
      ...
      if ((isset($response->error) && $response->error == '')) {
        ...
        if (isset($response->data)) {
          ...
          if (isset($response->errors->key[0])) {
            ...
            $return->message = esc_html__("Check your entered licese key.", "server-side-tagging-via-google-tag-manager-for-wordpress");
          }
        }
        return $return;
        return $return;
  ...
  public function siteVerification($postData)
  {
    try {
      ...
      $data = [
        'merchant_id' => sanitize_text_field($postData['merchant_id']),
        'website' => esc_url($postData['website_url']),
  ...
  public function convsst_get_gtm_account_list($postData)
  {
    try {
  ...
  public function convsst_create_gtm_container($postData)
  {
    try {
  ...
  public function convsst_run_gtm_automation($postData)
  {
    try {
  ...
  public function convsst_get_gtm_account_with_container($postData)
  {
    try {
      ...
      if (isset($response->error) && $response->error == false) {
        ...
        // update_option('convsst_options', serialize($ee_options_data));
  ...
  public function convsst_get_global_container_json($postData)
  {
    try {
  ...
  public function convsst_create_cloud_run($postData)
  {
    try {
