...
class ConvsstShoppingApi
{
    ...
    public function order_performance($from_date = '', $to_date = '', $limit = 5, $domain = '')
    {
        try {
    ...
    public function product_performance($date_range_type, $days = 0, $from_date = '', $to_date = '', $limit = 50, $offset = 0, $dimensionNamefilter = '')
    {
        try {
            $url = $this->apiDomain . '/actionable-dashboard/product-performance-report-ga4';
            $days_diff = 0;
            if ($date_range_type == 2) {
                $days_diff = strtotime($to_date) - strtotime($from_date);
                $days_diff = abs(round($days_diff / 86400));
    ...
    public function source_performance($date_range_type, $days = 0, $from_date = '', $to_date = '', $limit = 1000, $offset = 0)
    {
        try {
            $url = $this->apiDomain . '/actionable-dashboard/medium-performance-report-ga4';
            $days_diff = 0;
            if ($date_range_type == 2) {
                $days_diff = strtotime($to_date) - strtotime($from_date);
                $days_diff = abs(round($days_diff / 86400));
    ...
    public function campaign_performance($date_range_type, $days = 0, $from_date = '', $to_date = '', $limit = 5)
    {
        try {
    ...
    public function set_email_configurationGA4($subscription_id, $is_disabled, $custom_email = '', $email_frequency = '')
    {
        try {
            if ($is_disabled == '1') {
                $data = array('is_disabled' => $is_disabled, 'subscription_id' => $subscription_id);
            } else {
    ...
    public function save_all_reports($subscription_id,$start_date,$end_date, $domain, $ga4_analytic_account_id ='', $ga4_property_id ='', $google_ads_id='', $measurement_id='') 
    {
        try {
            ...
            //$curl_url = '/api/aireporting/save_all_reports';
            $header = array(
                "Authorization: Bearer $this->token",
                "Content-Type" => "application/json"
            );
    ...
    /*public function generate_ai_response($subscription_id, $key, $domain,$ai_flag = '0')
    {
        try {
            ...
            //$curl_url = '/api/aireporting/get-prompt-response';
            $header = array(
                "Authorization: Bearer $this->token",
                "Content-Type" => "application/json"
            );
