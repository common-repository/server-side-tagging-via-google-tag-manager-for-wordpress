...
if ( ! class_exists( 'Convsst_Category_Selector_Element' ) ) :
	class Convsst_Category_Selector_Element {
		/**
		...
		public static function category_mapping_row( $category, $level_indicator, $mode, $ee_prod_mapped_cats, $option) {
			...
			return '<div class="row catTermId termId_'.esc_attr($category->term_id).' ">
                <div class="col-6 mt-2 ">
                  <div class="form-group shop-category ps-3">
		...
		public static function category_mapping_selector( $identifier, $id, $start_visible, $ee_prod_mapped_cats, $option ) {
			...
			$category_levels = apply_filters( 'convsst_category_selector_level', 6 );
			$id = esc_attr($id);
			if(isset($ee_prod_mapped_cats[$id]['id']) && isset($ee_prod_mapped_cats[$id]['name']) && $ee_prod_mapped_cats[$id]['id'] && $ee_prod_mapped_cats[$id]['name']){
				...
				$html_code  = '<div id="category-selector-' . esc_attr($ident) . '" style="display:' . esc_attr($display) . '">
					...
					<select class="form-control" style="display:none;" id="' . esc_attr($ident) . '_0" catId="'.esc_attr($id).'" onchange="selectSubCategory(this)" iscategory="false">'.$option.'</select>';
				...
				$html_code  = '<div id="category-selector-' . esc_attr($ident) . '" style="display:' . esc_attr($display) . '">
					...
					<select style="width:100%" class="form-control select2" id="' . esc_attr($ident) . '_0" catId="'.esc_attr($id).'" onchange="selectSubCategory(this)" iscategory="false">
					'.$option.'
					</select>';
			...
			/*if (!class_exists('ConvsstShoppingApi')) {
	            require_once(__DIR__ . '/ConvsstShoppingApi.php');
	        }*/        
		...
		private static function category_description_data_item( $category_id ) {
			...
			$html_code = '<span aria-hidden="true">' . esc_html($category_description) . '</span>';

			return $html_code;
		}
	}
	// end of CONVSST_Category_Selector_Element class
endif;